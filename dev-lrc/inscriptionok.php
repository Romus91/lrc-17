	...
	if (filter_var(mysql_escape_string($_POST['email']), FILTER_VALIDATE_EMAIL))
	{
	   ...
	   //si ils sont les même on se connecte à la base de donnée
		  ...
		  { 
			...
			$date=time();
             $sql = 'INSERT INTO membre (login,pass,date,email) VALUES("'.mysql_escape_string($_POST['login']).'", "'.md5(mysql_escape_string($_POST['pass'])).'","'.$date.'", "'. mysql_real_escape_string($email).'")'; 
